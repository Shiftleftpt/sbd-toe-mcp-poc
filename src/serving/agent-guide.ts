...
export function generateReadingsBlock(): string {
  return [
    ...
    "",
    "**MacroProcess, capítulo e fase do SDLC são TRÊS SEGMENTAÇÕES PARALELAS do mesmo domínio,",
    "sem contenção entre si.** Um MP não «contém» capítulos: `traverses_bundles` é um PERCURSO",
    "publicado e ordenado. E a travessia MP↔fase do SDLC é **lacuna declarada** (parcial, não",
    "publicada): não a derives de capítulos nem de atribuições — a resposta declara-a como limite.",
    "",
    "**O `risk_level` é obrigatório onde a pergunta é «o que se aplica ao MEU caso»** — selecção,",
    "prepare, capacidade — e **opcional onde a pergunta é «o que o Manual diz»** (CONSULT). Não é",
    "flexibilização: é a fronteira entre seleccionar e conhecer."
  ].join("\n");
}
