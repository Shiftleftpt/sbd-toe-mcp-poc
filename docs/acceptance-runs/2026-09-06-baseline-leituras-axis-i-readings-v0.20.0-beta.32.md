# Eixo I — LEITURAS vs oráculo do lead — 2026-09-06-baseline-leituras — @shiftleftpt/sbd-toe-mcp@0.20.0-beta.32
...
Veredictos: **1 SERVIDO · 5 SERVIDO-MAL · 0 NÃO SERVIDO** (de 6).

| Caso | Leitura | Veredicto | Peças servidas | Superfícies usadas |
|---|---|---|---|---|
| GR-01 | IMPL | **SERVIDO-MAL** | 3/5 | get_sbd_toe_chapter_implementation_checklist, get_sbd_toe_chapter_brief, resolve_entities, query_sbd_toe_entities, get_guide_by_role, select_sbd_toe_requirements |
| GR-02 | CROSS-CHECK/PLAYBOOK | **SERVIDO-MAL** | 1/5 | map_sbd_toe_regulatory_activation, search_sbd_toe_manual, map_sbd_toe_regulatory_activation(PCI-DSS) |
| GR-03 | PROGRAMA | **SERVIDO-MAL** | 3/6 | plan_sbd_toe_rollout, query_sbd_toe_entities, select_sbd_toe_requirements |
...
| Peça do must-have | Servida | Evidência |
|---|---|---|
| playbook: mapa artigo→capítulo→acção | **não** | não encontrado |
| as 6 fases com marcos (M0-M2 … M12-M18) | **não** | sem entidade nem caminho estruturado para fases do playbook |
| checklist de leitura | **não** | sem caminho próprio |
| delimitação honesta (manual vs overlay/compliance) | sim | 15 áreas activadas |
| princípio declarado (cobre base AppSec; conformidade exige formalização) | **não** | não publicado como dado |

**O que falta para subir de estado:**

- playbook: mapa artigo→capítulo→acção — não encontrado
- as 6 fases com marcos (M0-M2 … M12-M18) — sem entidade nem caminho estruturado para fases do playbook
- checklist de leitura — sem caminho próprio
- princípio declarado (cobre base AppSec; conformidade exige formalização) — não publicado como dado
...
| macro-processos MP1–MP5 como dados | sim | encontrados |
